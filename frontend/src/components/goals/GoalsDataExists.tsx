      ...
      <Card>
        <CardHeader className="pb-4">
          <CardTitle className="text-2xl font-bold flex flex-row items-center justify-between">
            ...
            <AddGoalDialog goals={goals} setGoals={setGoals} />
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div
            ...
            {goals.map((goal, index) => (
              <div key={goal.id} className="space-y-2">
                ...
                <div className="flex justify-between text-sm pt-0.5">
                  <span>
                    {currencyNotation(goal.current)} /{" "}
                    {currencyNotation(goal.target)}
                  </span>
                  <span>
                ...
                <div className="flex justify-between text-xs pt-0.5 text-muted-foreground">
                  ...
                  <span>
                    {currencyNotation(goal.target - goal.current)} remaining
                  </span>
                </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="text-sm font-medium">
                {currencyNotation(totalTarget)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm font-medium">Total Saved:</span>
              <span className="text-sm font-medium">
                {currencyNotation(totalSaved)}
              </span>
            </div>
            <div className="flex justify-between">
