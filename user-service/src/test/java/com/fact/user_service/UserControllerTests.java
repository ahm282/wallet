...
class UserControllerTests {
  ...
  //        UserResponse user1 = new UserResponse(UUID.randomUUID(), "hansb", "Hans",
  // "Bartholomeus", "hasb@example.com", now);
  //        UserResponse user2 = new UserResponse(UUID.randomUUID(), "marcdb", "Marc", "De Boer",
  // "m.deboer@example.com", now);
  //
  ...
  //        Long now = System.currentTimeMillis();
  //        UserResponse user = new UserResponse(userId, "woutp", "Wout", "Peeters",
  // "woutp@gmail.com", now);
  //
  ...
  void shouldReturnNotFoundWhenUserNotExists() throws Exception {
    ...
    mockMvc.perform(get("/api/user/{id}", userId)).andExpect(status().isNotFound());
  ...
  //        Long now = System.currentTimeMillis();
  //        UserRequest userRequest = new UserRequest("robbedb", "Robbe", "De Busser",
  // "robbed@tm.com", "puddingmeneer");
  //        UserResponse userResponse = new UserResponse(UUID.randomUUID(), "robbedb", "Robbe", "De
  // Busser", "robbed@tm.com", now);
  //
  //
  ...
  //                        .content("{\"username\": \"robbedb\", \"email\": \"robbed@tm.com\",
  // \"password\": \"puddingmeneer\", \"firstName\": \"Robbe\", \"lastName\": \"De Busser\"}"))
  //                .andExpect(status().isOk())
  ...
  //        UUID userId = UUID.randomUUID();
  //        Long now = System.currentTimeMillis();
  //        UserRequest newMockUser = new UserRequest("arnep", "Arne", "Pelkmans",
  // "apelkmans@telecom.be", "topsecret");
  //
  //        // Create a UserResponse that the service would return after user creation
  //        UserResponse createdUserResponse = new UserResponse(userId, "arnep", "Arne", "Pelkmans",
  // "apelkmans@telecom.be", now);
  //
  ...
  //                        .content("{ \"username\": \"arnep\", \"firstName\": \"Arne\",
  // \"lastName\": \"Pelkmans\", \"email\": \"apelkmans@telecom.be\", \"password\": \"topsecret\"
  // }"))
  //                .andExpect(status().isCreated());  // Expecting 201 Created status after
  // creation
  //
  //        // Act: Simulate DELETE request to delete the user by ID
  //        mockMvc.perform(delete("/api/user/{id}", createdUserResponse.getId()))
  //                .andExpect(status().isOk());  // Expecting 200 OK status after successful
  // deletion
  //
  ...
  void shouldReturnNotFoundWhenUserToDeleteDoesNotExist() throws Exception {
    ...
    // Act & Assert
    mockMvc.perform(delete("/api/user/{id}", userId)).andExpect(status().isNotFound());
